...
const p1Artifacts = [
  { phase: "1.1", name: { en: "Toolchain and core-stack smoke", zh: "工具链与核心栈冒烟检查" }, run: "20260527T093100Z-phase-1-1-smoke", sha: "b5668d9", command: "make doctor / up-core / gen", href: "/case-studies/exactly-once-drills/results/phase-1.1-smoke.json" },
  { phase: "1.2", name: { en: "CDC correctness smoke", zh: "CDC 正确性冒烟检查" }, run: "20260527T133945Z-58e25cd1", sha: "b2434d1", command: "make test-cdc", href: "/case-studies/exactly-once-drills/results/phase-1.2-cdc-smoke.json" },
  { phase: "2.1", name: { en: "Five-failure reconciliation", zh: "五类故障对账" }, run: "20260527T151754Z-ef73a5a5", sha: "b2434d1", command: "make eo-verify", href: "/case-studies/exactly-once-drills/results/eo_reconciliation.json" },
  { phase: "2.2", name: { en: "Iceberg small-file rewrite", zh: "Iceberg 小文件重写" }, run: "20260527T155713Z-a88e18f6", sha: "ad7ad52", command: "make small-file-rewrite", href: "/case-studies/exactly-once-drills/results/iceberg_small_file_rewrite.json" },
  { phase: "2.3", name: { en: "Checkpoint pressure capture", zh: "检查点压力留证" }, run: "20260527T233135Z-0b65b846", sha: "ce084a5", command: "make ckpt-metrics", href: "/case-studies/exactly-once-drills/results/checkpoint_metrics.json" },
] as const;
  ...
  return (
    <ProjectProofSection title={dict.evidence} className="tinted-section">
      ...
      <div className="p1-pressure-section" aria-label={locale === "en" ? "Recorded historical checkpoint pressure evidence" : "已记录的历史检查点压力证据"}>
        ...
        <p className="evidence-link"><ArtifactLink href="/case-studies/exactly-once-drills/results/checkpoint_metrics.json">{locale === "en" ? "Open source JSON" : "查看源 JSON"}</ArtifactLink></p>
