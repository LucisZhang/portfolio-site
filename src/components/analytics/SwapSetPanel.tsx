...
export default function SwapSetPanel({ rows, approveThreshold, locale, sourceLabel, showObservedOutcomes = false }: { rows: SwapSetRow[]; approveThreshold: number; locale: Locale; sourceLabel: string; showObservedOutcomes?: boolean }) {
  ...
  return <section className="swap-set-panel" aria-labelledby="swap-set-title"><div className="analytics-pane-heading"><span id="swap-set-title">{locale === "en" ? "Baseline ↔ challenger swap set" : "Baseline ↔ challenger 换入换出集"}</span><code>{sourceLabel}</code></div><div><article><span>{locale === "en" ? "Baseline approvals" : "Baseline 批准数"}</span><strong>{baselineApproved.length}</strong></article><article><span>{locale === "en" ? "Challenger approvals" : "Challenger 批准数"}</span><strong>{challengerApproved.length}</strong></article><article><span>{locale === "en" ? "Challenger-only approvals" : "仅 Challenger 批准"}</span><strong>{challengerOnly}</strong></article><article><span>{locale === "en" ? "Baseline-only approvals" : "仅 Baseline 批准"}</span><strong>{baselineOnly}</strong></article><article className={expectedLossDelta > 0 ? "negative" : "positive"}><span>{locale === "en" ? "Expected-loss delta" : "预期损失差值"}</span><strong>{amount.format(expectedLossDelta)}</strong></article></div>{hasObservedOutcomes ? <div className="swap-set-observed" data-testid="swap-set-observed-rates"><span>{locale === "en" ? "Observed default rates in the loaded outcome window" : "已载入结果窗口中的观察违约率"}</span><article><small>{locale === "en" ? "Challenger-only" : "仅 Challenger 批准"}</small><strong>{rateLabel(challengerOnlyRate, challengerOnly)}</strong></article><article><small>{locale === "en" ? "Baseline-only" : "仅 Baseline 批准"}</small><strong>{rateLabel(baselineOnlyRate, baselineOnly)}</strong></article></div> : null}<p>{locale === "en" ? "Counts and expected loss are recomputed at the current approval threshold. This is a policy comparison, not model superiority evidence." : "数量与预期损失按当前批准阈值重算；这是策略比较，不构成模型优越性证据。"}</p></section>;
}
