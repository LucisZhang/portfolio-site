      ...
      {open ? (
        <div className="wechat-modal" role="dialog" aria-modal="true" aria-labelledby="phone-title" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
          <div className="wechat-card">
            <button ref={closeRef} type="button" className="wechat-close" onClick={close} aria-label={locale === "en" ? "Close phone number" : "关闭电话号码"}>{locale === "en" ? "Close" : "关闭"}</button>
            <p className="eyebrow">{locale === "en" ? "Phone" : "电话"}</p>
            <h2 id="phone-title">{locale === "en" ? "Contact by phone" : "电话联系"}</h2>
            <div className="wechat-id-row"><span><strong>{siteIdentity.profiles.phone}</strong></span><button type="button" onClick={() => void copyNumber()}>{copied ? (locale === "en" ? "Copied" : "已复制") : (locale === "en" ? "Copy number" : "复制号码")}</button></div>
          </div>
        </div>
      ) : null}
    </>
