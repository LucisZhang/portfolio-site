...
const scenarioKinds: Record<string, { en: string; zh: string }> = {
  ...
  "SYN-SCHEMA-02": { en: "Database schema change", zh: "数据库 Schema 变更" },
...
const evidenceDetailLabels: Record<string, { en: string; zh: string }> = {
  ...
  runbook_status: { en: "Runbook status", zh: "Runbook 状态" },
...
const verifiedEvidence = [
  ...
  { id: "findings", stages: [6, 7, 8], title: { en: "Consistency findings", zh: "一致性审计发现" }, href: "/case-studies/release-guardian/data/findings.csv", date: "2026-07-12", evidenceClass: { en: "13 sanitized findings", zh: "13 项脱敏审计发现" }, result: { en: "Documents the live/stub wording correction and publication boundaries.", zh: "记录 live/stub 文案纠正和公开边界。" }, boundary: { en: "Finding summaries only; private traces and internal paths remain withheld.", zh: "仅公开发现摘要；私有 trace 与内部路径不公开。" } },
  { id: "live", stages: [], title: { en: "Funded live evaluation", zh: "付费在线评估" }, href: "/case-studies/release-guardian/data/evaluation-live.csv", date: "2026-07-11", evidenceClass: { en: "measured historical evaluation", zh: "历史实测评估" }, result: { en: "44 scenarios × 3 trials = 132 graph runs; 30 / 44 outcome_pass:false in the strict all-trials view.", zh: "44 个场景 × 3 次试验 = 132 次图运行；严格全试验视图中 30 / 44 的 outcome_pass:false。" }, boundary: { en: "This is historical evaluation evidence, not an output of the synthetic replay above.", zh: "这是历史评估证据，不是上方合成回放的输出。" } },
...
export default function ReleaseChangeReplay() {
  ...
  useEffect(() => {
    ...
    fetch(SCENARIO_URL).then((response) => {
      ...
      if (active) setLoadError(reason instanceof Error ? reason.message : "Synthetic replay could not be loaded.");
    });
    return () => { active = false; };
  }, []);
  ...
  const stageLabels = locale === "en"
    ...
    : ["变更接收", "代码证据", "Schema 证据", "API 证据", "运维证据", "风险判断", "发布计划", "审批门禁", "审计记录"];
  ...
  if (loadError) return <div className="release-replay-loading error"><CircleAlert aria-hidden="true" />{loadError}</div>;
  ...
  return (
    <section className="release-replay" data-testid="release-change-replay" aria-labelledby="release-replay-title">
      <header className="release-replay-header">
        <div><p className="eyebrow">{locale === "en" ? "Synthetic scenario / deterministic replay" : "合成场景 / 确定性回放"}</p><h3 id="release-replay-title">Sanitized Change Review Replay</h3><p>{locale === "en" ? "Follow a fictional change from intake through parallel evidence, deterministic risk rules, release planning, and human approval." : "跟随一个虚构变更，从接收到并行证据、确定性风险规则、发布计划和人工审批。"}</p></div>
        <div className="release-fixed-disclosure"><ShieldCheck aria-hidden="true" /><strong>{data.fixed_disclosure}</strong></div>
      </header>

      <div className="release-scenario-tabs" role="tablist" aria-label={locale === "en" ? "Synthetic change scenario" : "合成变更场景"}>
        {data.scenarios.map((item) => <button type="button" role="tab" aria-selected={item.id === scenario.id} key={item.id} onClick={() => selectScenario(item.id)}><span>{scenarioKinds[item.id]?.[locale] || item.kind}</span><code>{item.id}</code></button>)}
      </div>

      <div className="release-intake-strip">
        <div><span>{locale === "en" ? "Synthetic summary" : "合成摘要"}</span><strong>{scenario.summary}</strong></div>
        <div><span>{locale === "en" ? "Declared owner" : "声明负责人"}</span><strong>{scenario.declared_owner}</strong></div>
        <div><span>{locale === "en" ? "Expected impact" : "预期影响"}</span><strong>{scenario.expected_impact}</strong></div>
        <div className="wide"><span>{locale === "en" ? "Proposed change" : "拟议变更"}</span><strong>{scenario.proposed_change}</strong><p>{scenario.affected_components.map((component) => <code key={component}>{component}</code>)}</p></div>
      </div>
      ...
      <div className="release-replay-workspace" aria-live="polite">
        <div className="release-stage-main">
          ...
          {step === 0 && <div className="release-fact-panel"><p><span>{locale === "en" ? "synthetic fact" : "合成事实"}</span>{scenario.summary}</p><p><span>{locale === "en" ? "synthetic fact" : "合成事实"}</span>{scenario.proposed_change}</p><p><span>{locale === "en" ? "UI explanation" : "界面说明"}</span>{locale === "en" ? "The intake is fictional and exists only to demonstrate the review contract." : "该 intake 完全虚构，仅用于演示审查契约。"}</p></div>}

          {currentRetriever && <div className="release-retriever-result"><div><GitPullRequest aria-hidden="true" /><span>{locale === "en" ? "Synthetic structured result" : "合成结构化结果"}</span><strong>{retrieverOrder[step - 1]} {locale === "en" ? "evidence" : "证据"}</strong><code>{currentRetriever.status}</code></div><dl className="release-evidence-detail-grid">{Object.entries(currentRetriever.details).map(([key, value]) => <div key={key} className={key === "source_boundary" ? "withheld" : ""}><dt>{evidenceDetailLabels[key]?.[locale] ?? key.replaceAll("_", " ")}</dt><dd>{value}</dd></div>)}</dl><ul>{currentRetriever.items.map((item) => <li key={item}>{item}</li>)}</ul><p><strong>{locale === "en" ? "Supporting evidence" : "支持证据"}</strong>{currentRetriever.support}</p></div>}

          {step === 5 && <div className="release-risk-grid"><div className={`release-risk-score ${risk.level}`}><span>{locale === "en" ? "deterministic rule result" : "确定性规则结果"}</span><strong>{risk.level}</strong><code>{risk.score} / 100</code></div><div className="release-rule-list"><h5>{locale === "en" ? "Matched rules" : "命中规则"}</h5>{risk.matches.length ? risk.matches.map(([name, points]) => <p key={name}><span>+{points}</span><strong>{name.replaceAll("_", " ")}</strong></p>) : <p><span>+0</span><strong>{locale === "en" ? "No weighted risk rule matched" : "没有命中加权风险规则"}</strong></p>}</div><div className="release-blocker-list"><h5>{locale === "en" ? "Blocking conditions" : "阻断条件"}</h5>{risk.blockers.length ? risk.blockers.map((blocker) => <p key={blocker}><CircleAlert aria-hidden="true" />{blocker}</p>) : <p><Check aria-hidden="true" />{locale === "en" ? "No deterministic blocker." : "没有确定性阻断项。"}</p>}</div><p className="release-risk-explanation"><span>{locale === "en" ? "UI explanation" : "界面说明"}</span>{locale === "en" ? "The score is the visible sum of fixed rule weights. This is not model reasoning or hidden chain-of-thought." : "分数是固定规则权重的可见求和，不是模型推理或隐藏思维链。"}</p></div>}

          {step === 6 && <div className="release-plan-grid"><PlanColumn title={locale === "en" ? "Rollout" : "发布步骤"} items={scenario.rollout} /><PlanColumn title={locale === "en" ? "Rollback" : "回滚步骤"} items={scenario.rollback} /><PlanColumn title={locale === "en" ? "Verification" : "校验检查"} items={scenario.verification_checks} /><PlanColumn title={locale === "en" ? "Stop when" : "停止条件"} items={scenario.stopping_conditions} /></div>}

          {step === 7 && <div className="release-approval-gate"><ShieldCheck aria-hidden="true" /><span>{locale === "en" ? "Human approval interrupt" : "人工审批中断"}</span><h4>{risk.blockers.length ? (locale === "en" ? "Deterministic recommendation: reject until evidence gaps close" : "确定性建议：证据缺口关闭前拒绝") : (locale === "en" ? "Deterministic recommendation: review and approve" : "确定性建议：审查后批准")}</h4><p>{locale === "en" ? "The visitor makes the decision. Both outcomes create a synthetic audit record and can be compared." : "决定由访客作出。两种结果都会生成合成审计记录，并可进行比较。"}</p><div><button type="button" className="approve" onClick={() => decide("approved")}><Check aria-hidden="true" />{locale === "en" ? "Approve" : "批准"}</button><button type="button" className="reject" onClick={() => decide("rejected")}><X aria-hidden="true" />{locale === "en" ? "Reject" : "拒绝"}</button></div></div>}

          {step === 8 && <div className="release-audit-grid">{decisions.map((decision) => <article key={decision} className={decision}><span>{locale === "en" ? "synthetic audit record" : "合成审计记录"}</span><strong>{locale === "en" ? decision : decision === "approved" ? "已批准" : "已拒绝"}</strong><dl><div><dt>ID</dt><dd>{auditId(scenario, decision)}</dd></div><div><dt>{locale === "en" ? "Scenario" : "场景"}</dt><dd>{scenario.id}</dd></div><div><dt>{locale === "en" ? "Fixed time" : "固定时间"}</dt><dd>{scenario.synthetic_timestamp}</dd></div><div><dt>{locale === "en" ? "Risk" : "风险"}</dt><dd>{risk.level} / {risk.score}</dd></div><div><dt>{locale === "en" ? "Requirements" : "审批要求"}</dt><dd>{scenario.approval_requirements.join(", ")}</dd></div></dl></article>)}</div>}
        </div>
        ...
        <aside className="release-evidence-progress">
          ...
          {retrieverOrder.map((name, index) => {
            ...
            return <div key={name} className={reached ? "reached" : ""}><strong>{name}</strong><code>{reached ? scenario.retrievers[name].status : "queued"}</code><p>{reached ? scenario.retrievers[name].items.length : 0} {locale === "en" ? "structured items" : "条结构化结果"}</p></div>;
          })}
          <div className="release-evidence-gap"><strong>{locale === "en" ? "Missing evidence" : "缺失证据"}</strong><code>{step >= 5 ? scenario.missing_evidence.length : "-"}</code><p>{step >= 5 ? (scenario.missing_evidence.join("; ") || (locale === "en" ? "None" : "无")) : (locale === "en" ? "Evaluated after retrieval" : "检索后评估")}</p></div>
        </aside>
      </div>
      ...
      <footer className="release-replay-footer"><div><span>{locale === "en" ? "Fixture seed" : "夹具 seed"}</span><code>{data.seed}</code></div><div><span>{locale === "en" ? "Classification" : "分类"}</span><strong>{data.classification.join(" / ")}</strong></div><ArtifactLink href={SCENARIO_URL}>{locale === "en" ? "Explore scenario data" : "浏览合成场景数据"}</ArtifactLink></footer>
    </section>
  );
}

function PlanColumn({ title, items }: { title: string; items: string[] }) {
  return <section><h5>{title}</h5><ol>{items.map((item, index) => <li key={item}><span>{String(index + 1).padStart(2, "0")}</span>{item}</li>)}</ol></section>;
}
