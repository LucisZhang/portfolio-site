...
const verifiedEvidence = [
  { id: "live", stages: [], title: { en: "Funded live evaluation", zh: "付费在线评估" }, href: "/case-studies/release-guardian/data/evaluation-live.csv", date: "2026-07-11", evidenceClass: { en: "measured historical evaluation", zh: "历史实测评估" }, result: { en: "8 / 8 aggregate gates passed across 44 scenarios × 3 trials = 132 graph runs; the strict all-trials view also surfaces 30 / 44 outcome_pass:false.", zh: "44 个场景 × 3 次试验 = 132 次图运行，8 / 8 聚合门禁通过；严格全试验视图同时呈现 30 / 44 的 outcome_pass:false。" } },
  { id: "stub", stages: [], title: { en: "Deterministic stub evaluation", zh: "确定性 stub 评估" }, href: "/case-studies/release-guardian/data/evaluation-stub.csv", date: "2026-07-11", evidenceClass: { en: "deterministic historical stub", zh: "历史确定性 stub" }, result: { en: "132 repeatable stub graph runs with 15 / 44 flagged in the strict all-trials view.", zh: "132 次可重复的 stub 图运行；严格全试验视图标记 15 / 44。" } },
  { id: "architecture", stages: [1, 2, 3, 4], title: { en: "Sanitized architecture", zh: "脱敏系统架构" }, href: "/case-studies/release-guardian/architecture.mmd", date: "2026-07-12", evidenceClass: { en: "presentation-layer derivative", zh: "展示层衍生物" }, result: { en: "Maps the public review contract across intake, retrieval, risk, planning, approval, and audit.", zh: "呈现从变更接收、检索、风险评估、计划、审批到审计的公开审查契约。" } },
  { id: "risk", stages: [5], title: { en: "Risk guardrail capture", zh: "风险门禁截图" }, href: "/case-studies/release-guardian/screenshots/risk-guardrail.png", date: "2026-07-12", evidenceClass: { en: "sanitized historical capture", zh: "脱敏历史截图" }, result: { en: "Shows the risk factors, fixed weights, blockers, and human approval gate in one review surface.", zh: "在同一审查界面呈现风险因子、固定权重、阻断项与人工审批门禁。" } },
  { id: "findings", stages: [6, 7, 8], title: { en: "Consistency findings", zh: "一致性审计发现" }, href: "/case-studies/release-guardian/data/findings.csv", date: "2026-07-12", evidenceClass: { en: "13 sanitized findings", zh: "13 项脱敏审计发现" }, result: { en: "Records 13 consistency findings and the resulting live/stub wording corrections.", zh: "记录 13 项一致性发现，以及由此完成的在线/stub 文案纠正。" } },
] as const;
  ...
  return (
    <section className="release-replay" data-testid="release-change-replay" aria-labelledby="release-replay-title">
      <header className="release-replay-header">
        ...
        <div className="release-fixed-disclosure"><ShieldCheck aria-hidden="true" /><strong>{locale === "en" ? "A repeatable review workflow built from fixed scenarios and deterministic rules." : "基于固定场景与确定性规则构建的可重复审查工作流。"}</strong></div>
      </header>
      ...
      <div className="release-replay-body">
        ...
        <section className="release-verified-rail" aria-labelledby="release-verified-title">
          <header><div><p className="eyebrow">{locale === "en" ? "Verified run record" : "已核验运行记录"}</p><h4 id="release-verified-title">{locale === "en" ? "Compare the replay with historical results" : "将回放与历史结果直接对照"}</h4></div><p>{locale === "en" ? "Inspect the interactive workflow beside the dated live and deterministic-stub result ledgers." : "在交互工作流旁直接检查带日期的在线实测与确定性 stub 结果台账。"}</p></header>
          <div className="release-verified-rail-list">
            <article className="active synthetic"><span>{locale === "en" ? "Current replay" : "当前回放"}</span><strong>{scenario.id}</strong><dl><div><dt>{locale === "en" ? "Date" : "日期"}</dt><dd>{scenario.synthetic_timestamp} ({locale === "en" ? "fictional" : "虚构"})</dd></div><div><dt>{locale === "en" ? "Class" : "类别"}</dt><dd>{locale === "en" ? "synthetic / deterministic" : "合成 / 确定性"}</dd></div><div><dt>{locale === "en" ? "Result" : "结果"}</dt><dd>{locale === "en" ? "Interactive review path completed." : "交互式审查路径已完成。"}</dd></div></dl><ArtifactLink href={SCENARIO_URL}>{locale === "en" ? "Explore synthetic scenario" : "浏览合成场景"}</ArtifactLink></article>
            {verifiedEvidence.map((item) => <article key={item.id} className={(item.stages as readonly number[]).includes(step) ? "active" : ""}><span>{item.stages.length ? (locale === "en" ? "Related to this stage" : "与当前阶段相关") : (locale === "en" ? "Historical evaluation" : "历史评估")}</span><strong>{item.title[locale]}</strong><dl><div><dt>{locale === "en" ? "Date" : "日期"}</dt><dd>{item.date}</dd></div><div><dt>{locale === "en" ? "Class" : "类别"}</dt><dd>{item.evidenceClass[locale]}</dd></div><div><dt>{locale === "en" ? "Result" : "结果"}</dt><dd>{item.result[locale]}</dd></div></dl><ArtifactLink href={item.href}>{locale === "en" ? "Open run record" : "查看运行记录"}</ArtifactLink></article>)}
          </div>
        </section>
      </div>

      <footer className="release-replay-footer"><div><span>{locale === "en" ? "Fixture seed" : "夹具 seed"}</span><code>{data.seed}</code></div><div><span>{locale === "en" ? "Workflow" : "工作流"}</span><strong>{locale === "en" ? "fixed inputs / deterministic rules / human approval" : "固定输入 / 确定性规则 / 人工审批"}</strong></div><ArtifactLink href={SCENARIO_URL}>{locale === "en" ? "Explore scenario data" : "浏览合成场景数据"}</ArtifactLink></footer>
    </section>
  );
}
