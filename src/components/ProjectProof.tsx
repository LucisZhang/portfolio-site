...
function AnalyticsMigrationProof() {
  ...
  return (
    <ProjectProofSection title={dict.evidence} className="tinted-section">
      <div className="analytics-migration"><p className="eyebrow">{locale === "en" ? "Legacy route migration" : "旧路由迁移"}</p><h3>{locale === "en" ? "Analytics Tandem has been split into two operable case studies" : "原 Analytics Tandem 已拆分为两个可独立运行的案例研究"}</h3><p>{locale === "en" ? "This compatibility page preserves the old URL. Choose the rebuilt decision workflow that matches your review." : "此兼容页面保留旧 URL。请选择符合评审方向的重建决策流程。"}</p><div><LocaleLink href="/projects/margin-control-tower"><strong>Margin Control Tower</strong><span>{locale === "en" ? "Analytics engineering, governed margin diagnosis, scenario verification" : "分析工程、治理约束下的毛利诊断、情景验证"}</span></LocaleLink><LocaleLink href="/projects/credit-policy-desk"><strong>Credit Policy Desk</strong><span>{locale === "en" ? "Risk calibration, expected loss, policy thresholds, monitoring" : "风险校准、预期损失、策略阈值、监控"}</span></LocaleLink></div></div>
...
// via the shared project catch-all, now src/app/projects/[slug]/page.tsx),
// so none of the four ever reaches ProjectProof. Task L1 gives "release-guardian" the same treatment
// (src/components/guardian/GuardianPage.tsx, via its own literal route
// src/app/projects/release-guardian/page.tsx) -- ReleaseGuardianProof.tsx and
