...
export function PolicyTerminal({ variant }: { variant: "compact" | "full" }) {
  ...
  const [misroutePosition, setMisroutePosition] = useState(DEFAULT_MISROUTE_INDEX);
  const misroutePositionRef = useRef(DEFAULT_MISROUTE_INDEX);
  const misrouteAnimation = useRef<number | undefined>(undefined);
  const [copiedSyntax, setCopiedSyntax] = useState(false);
  const copiedTimer = useRef<number | undefined>(undefined);
  useEffect(() => () => {
    window.clearTimeout(copiedTimer.current);
    if (misrouteAnimation.current !== undefined) cancelAnimationFrame(misrouteAnimation.current);
  }, []);
  const [thresholdIndex, setThresholdIndex] = useState(DEFAULT_THRESHOLD_INDEX);

  const misrouteIndex = Math.max(0, Math.min(matrix.misrouteCosts.length - 1, Math.round(misroutePosition)));
  ...
  function updateMisroutePosition(next: number) {
    const clamped = Math.max(0, Math.min(matrix.misrouteCosts.length - 1, next));
    misroutePositionRef.current = clamped;
    setMisroutePosition(clamped);
  }

  function stopMisrouteAnimation() {
    if (misrouteAnimation.current !== undefined) {
      cancelAnimationFrame(misrouteAnimation.current);
      misrouteAnimation.current = undefined;
    }
  }

  function animateMisrouteTo(target: number) {
    stopMisrouteAnimation();
    const clampedTarget = Math.max(0, Math.min(matrix.misrouteCosts.length - 1, target));
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      updateMisroutePosition(clampedTarget);
      return;
    }
    const start = misroutePositionRef.current;
    ...
    const tick = (now: number) => {
      ...
      updateMisroutePosition(start + (clampedTarget - start) * eased);
      if (elapsed < 1) misrouteAnimation.current = requestAnimationFrame(tick);
      else misrouteAnimation.current = undefined;
    };
    misrouteAnimation.current = requestAnimationFrame(tick);
  }

  function handleMisrouteKeyDown(event: ReactKeyboardEvent<HTMLInputElement>) {
    const current = Math.round(misroutePositionRef.current);
    let target: number | undefined;
    if (["ArrowRight", "ArrowUp", "PageUp"].includes(event.key)) target = current + 1;
    if (["ArrowLeft", "ArrowDown", "PageDown"].includes(event.key)) target = current - 1;
    if (event.key === "Home") target = 0;
    if (event.key === "End") target = matrix.misrouteCosts.length - 1;
    if (target === undefined) return;
    event.preventDefault();
    animateMisrouteTo(target);
  }
    ...
    <InstrumentFrame variant={variant}>
      <div className="triage-terminal" data-triage-terminal data-variant={variant}>
        <div className="triage-step triage-step-control" data-step="control">
          ...
          <FrontierChart misroutePosition={misroutePosition} thresholdIndex={thresholdIndex} />
          ...
          <div className="triage-slider-row">
            ...
            <input
              ...
              onChange={(event) => {
                stopMisrouteAnimation();
                updateMisroutePosition(Number(event.target.value));
              }}
              onPointerUp={() => animateMisrouteTo(Math.round(misroutePositionRef.current))}
              onPointerCancel={() => animateMisrouteTo(Math.round(misroutePositionRef.current))}
              onBlur={() => animateMisrouteTo(Math.round(misroutePositionRef.current))}
              onKeyDown={handleMisrouteKeyDown}
            />
            <span className="triage-slider-ticks" aria-hidden="true">
              {matrix.misrouteCosts.map((cost) => <i key={cost} />)}
            </span>
