...
import { numericTokens } from "../../scripts/lib/ask-authored-answers.mjs";
...
export interface AssistantCitation {
  ...
  url?: string;
  /** Section id on the destination project page (e.g. "exhibit-05"), authored-citation only. */
  anchor?: string;
}
...
export function retrieveAssistantKnowledge(
  ...
  // filling the bounded remainder with the existing relevance ranking. A
  // question that names a specific metric or file wants the pinned repo
  // evidence, not the site's own overview prose about itself — only prefer
  // the in-site chunk when the question is asking generally about a project.
  const detailQuestion = numericTokens(question).length > 0
    || /[\w-]+\.(?:json|md|py|ts|tsx|mjs|mmd)\b/u.test(question);
  const namedPriority = mentionedProjectIds(question).flatMap((id) => {
    ...
    if (detailQuestion) return matches.slice(0, 2);
