...
//
// The grouping governs the index and the chain ONLY. URLs are untouched by
// it: every stop's href is the project's own flat route, /projects/<slug>
// (Task A2), so /projects/crossover-study legitimately shows a
// MEASURE & DECIDE crumb even though the project's track is engineering.
//
...
import navigationProjects from "../data/generated/project-navigation.json" with { type: "json" };
const routableProjects = navigationProjects as Pick<Project, "slug" | "title" | "navigationLabel" | "glossZh">[];
...
export const circuitGroups: CircuitGroup[] = [
  ...
  },
  {
    ...
    members: ["triage-router", "crossover-study", "margin-control-tower", "credit-policy-desk", "groupconv-atlas"],
  },
];
...
const circuitBlurbEn: Record<string, string> = {
  "groupconv-atlas": "kernel speedups with their boundaries",
...
export interface CircuitStop {
  slug: ProjectId;
  project: Pick<Project, "slug" | "title" | "navigationLabel" | "glossZh">;
