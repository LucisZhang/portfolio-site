...
const requiredRoutes = (args.get("--required-routes") || "/projects/analytics-tandem")
  .split(",")
...
async function runInteraction(page, route, locale, viewport, pdfFixture, report) {
  ...
  try {
    if (route === "/projects/exactly-once-drills") {
    ...
    } else if (route === "/projects/release-guardian") {
    ...
    } else if (route === "/projects/rag-quality-lab") {
    ...
    } else if (route === "/projects/margin-control-tower") {
    ...
    } else if (route === "/projects/credit-policy-desk") {
      ...
      await verify.locator("code").waitFor({ state: "visible" });
      await screenshot("receipt-verified");
    } else if (route === "/projects/privacy-preflight") {
