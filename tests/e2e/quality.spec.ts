...
test("representative workflows remain keyboard-operable with reduced motion", async ({ page }, testInfo) => {
  ...
  await page.goto("/projects/exactly-once-drills", { waitUntil: "networkidle" });
  ...
  await page.goto("/projects/credit-policy-desk", { waitUntil: "networkidle" });
...
test("pass indicators use ok while small hover text keeps accessible accent contrast", async ({ page }, testInfo) => {
  ...
  await page.goto("/projects/release-guardian", { waitUntil: "networkidle" });
...
test("footer contact returns to the top of the localized homepage contact section", async ({ page }, testInfo) => {
  ...
  // "/ai/release-guardian?lang=zh" (its URL at the time; the page is served
  // at "/projects/release-guardian" today) via the legacy-shell route's
  // LegacyRailTools contact link. release-guardian is now a standalone
  // route (src/app/projects/release-guardian/page.tsx, guardianRail.ts) built on
...
test("artifact viewer preserves the shareable Chinese locale and project return URL", async ({ page }, testInfo) => {
  ...
  await page.goto("/artifact?src=/case-studies/exactly-once-drills/README.md&from=/projects/exactly-once-drills&lang=zh", { waitUntil: "networkidle" });
  await expect(page.locator(SEL.html)).toHaveAttribute("lang", "zh-CN");
  await expect(page.getByRole("link", { name: "返回项目" })).toHaveAttribute("href", /projects\/exactly-once-drills\?lang=zh$/);
...
test("root, project, and artifact metadata follow the active locale", async ({ page }, testInfo) => {
  ...
  // example still-legacy project route (URLs in this paragraph are the ones
  // that were live at the time -- the /ai, /engineering and /analytics
  // project URLs were retired later, by this branch's flat-route move, and
  // now 308 to /projects/<slug>; see tests/e2e/redirects.spec.ts).
  // rag-quality-lab is now a standalone route
  // (src/app/projects/rag-quality-lab/page.tsx) built on
  ...
  await page.goto("/projects/frontier-forge?lang=zh", { waitUntil: "networkidle" });
