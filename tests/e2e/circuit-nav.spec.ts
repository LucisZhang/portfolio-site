...
//   - every one of the 11 standalone project pages carries the top strip
//     (home crumb + prev/next with family context) and the bottom block,
//     with prev/next pointing at the correct circular neighbours;
//   - the colophon lists all 11 projects grouped 2/4/5 by the family
...
// a contiguous run and the index's global numbers climb 01..11 straight
// down the three columns; that agreement is asserted here so it cannot
// drift silently.
// The exhaustive 11-page x 2-locale sweep runs on the desktop project only
...
async function assertCircuitPage(page: Page, index: number, locale: Locale) {
  ...
  // Colophon: all 11 listed, grouped, exactly the current page marked.
  ...
  await expect(colophon.locator(".circuit-colophon-track")).toHaveText(locale === "zh"
    ? ["构建与运行 — 02", "把关与验证 — 04", "度量与决策 — 05"]
    : ["BUILD & RUN — 02", "GUARD & VERIFY — 04", "MEASURE & DECIDE — 05"]);
  ...
  await expect(colophon.locator(".circuit-colophon-num")).toHaveText(["01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11"]);
  ...
  expect(await groupBlocks.evaluateAll((nodes) => nodes.map((node) => node.getAttribute("tabindex"))))
    .toEqual(["-1", "-1", "-1"]);
  for (const [id, size] of [["build-run", 2], ["guard-verify", 4], ["measure-decide", 5]] as const) {
