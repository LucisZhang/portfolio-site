...
// next.config.ts permanently redirects the track indexes and the legacy
// analytics-tandem slug to "/", so they are the home page under another URL:
// asserted as redirects, not counted as covered surfaces.
const REDIRECTED_ROUTES = ["/ai", "/engineering", "/analytics", "/analytics/analytics-tandem"];
...
test("track indexes and the legacy slug redirect to the home page (covered there)", async ({ page }) => {
...
test("policy terminal copy label survives copy, restore and a locale switch without React errors", async ({ page, context }) => {
  ...
  const problems = watchConsole(page);
  await page.goto("/ai/triage-router?lang=zh", { waitUntil: "networkidle" });
  await page.waitForFunction(() => document.documentElement.lang === "zh-CN");
