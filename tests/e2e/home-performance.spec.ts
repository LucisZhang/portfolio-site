...
test("home loads the full catalog, question bank and circuit only at their entry points", async ({ page }) => {
  ...
  // This is a KEY INTO the generated question bank, not a site URL. Task A5
  // rekeyed assistant-knowledge/question-bank.json on the flat routes and
  // regenerated this artifact, so the key is now "/projects/triage-router".
  const bank = scriptsContaining(questionBank["/projects/triage-router"].questions[0].id);
  ...
  await expect(page.locator("[data-colophon-item]")).toHaveCount(11);
...
test("project circuit links remain in the server document without JavaScript", async ({ browser }, testInfo) => {
  ...
  const destination = circuitEntryForPath("/projects/frontier-forge")?.next;
  if (!destination) throw new Error("frontier-forge must have a next circuit stop");
  const context = await browser.newContext({ javaScriptEnabled: false });
  try {
    const page = await context.newPage();
    await page.goto(`${testInfo.project.use.baseURL}/projects/frontier-forge`);
    await expect(page.locator("[data-circuit-top]")).toBeVisible();
    await expect(page.locator("[data-colophon-item]")).toHaveCount(11);
