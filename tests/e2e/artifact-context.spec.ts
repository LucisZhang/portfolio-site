...
const cases = [
  { file: "privacy-preflight/image-synthetic-input.png", project: { en: "Privacy Preflight", zh: "Privacy Preflight" }, route: "/projects/privacy-preflight", type: "PNG", viewer: ".artifact-image-viewer img" },
  { file: "exactly-once-drills/media/phase-1.4-dashboard.jpg", project: { en: "Exactly-Once Drills", zh: "Exactly-Once Drills" }, route: "/projects/exactly-once-drills", type: "JPG", viewer: ".artifact-image-viewer img" },
  { file: "frontier-forge/architecture.svg", project: { en: "Frontier Forge", zh: "Frontier Forge" }, route: "/projects/frontier-forge", type: "SVG", viewer: ".artifact-image-viewer img" },
  { file: "privacy-preflight/pdf-synthetic-redacted.pdf", project: { en: "Privacy Preflight", zh: "Privacy Preflight" }, route: "/projects/privacy-preflight", type: "PDF", viewer: ".artifact-pdf-canvas canvas" },
  { file: "rag-quality-lab/claim-registry.json", project: { en: "RAG Quality Lab", zh: "RAG Quality Lab" }, route: "/projects/rag-quality-lab", type: "JSON", viewer: ".json-tree" },
  { file: "release-guardian/data/findings.csv", project: { en: "Release Guardian", zh: "Release Guardian" }, route: "/projects/release-guardian", type: "CSV", viewer: ".artifact-table-scroll" },
  { file: "exactly-once-drills/README.md", project: { en: "Exactly-Once Drills", zh: "Exactly-Once Drills" }, route: "/projects/exactly-once-drills", type: "MD", viewer: ".artifact-markdown" },
  { file: "margin-control-tower/architecture.mmd", project: { en: "Margin Control Tower", zh: "Margin Control Tower" }, route: "/projects/margin-control-tower", type: "MMD", viewer: ".artifact-mermaid-svg svg" },
  { file: "privacy-preflight/downloads/CPython-LICENSE.txt", project: { en: "Privacy Preflight", zh: "Privacy Preflight" }, route: "/projects/privacy-preflight", type: "TXT", viewer: ".artifact-raw-source" },
  { file: "credit-policy-desk/policy-contract.json", project: { en: "Credit Policy Desk", zh: "Credit Policy Desk" }, route: "/projects/credit-policy-desk", type: "JSON", viewer: ".json-tree" },
  { file: "crossover-study/workbench/queries.json", project: { en: "Crossover Study", zh: "Crossover Study" }, route: "/projects/crossover-study", type: "JSON", viewer: ".json-tree" },
  { file: "triage-router/strategy-cards.json", project: { en: "Triage Router", zh: "Triage Router" }, route: "/projects/triage-router", type: "JSON", viewer: ".json-tree" },
];
...
test("return and locale are canonical across aliases, nested languages, duplicates, and reload", async ({ page }) => {
  ...
  await expect(page.locator(".back-link")).toHaveAttribute("href", "/projects/exactly-once-drills");
  await page.reload();
  await expect(page.locator("html")).toHaveAttribute("lang", "en");
  await page.goto(url(rag, { from: "/projects/frontier-forge", lang: "zh-CN" }));
  await expect(page.locator("html")).toHaveAttribute("lang", "en");
  await expect(page.locator(".back-link")).toHaveAttribute("href", "/projects/rag-quality-lab");
  await page.goto(url(rag, { lang: "zh" }) + "&lang=en&from=//evil.test&from=/");
  await expect(page.locator("html")).toHaveAttribute("lang", "en");
  await expect(page.locator(".back-link")).toHaveAttribute("href", "/projects/rag-quality-lab");
  await page.getByRole("button", { name: "中", exact: true }).click();
  await expect(page.locator(".back-link")).toHaveAttribute("href", "/projects/rag-quality-lab?lang=zh");
  await page.locator(".back-link").click();
  await expect(page).toHaveURL(/\/projects\/rag-quality-lab\?lang=zh$/);
});
...
test("nested file links and browser history clear stale project, search, sections, and language", async ({ page }) => {
  ...
  await expect(page.locator("html")).toHaveAttribute("lang", "en");
  await expect(page.locator(".back-link")).toHaveAttribute("href", "/projects/frontier-forge");
