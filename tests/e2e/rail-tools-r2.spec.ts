...
test.describe("Bilingual project rail navigation", () => {
  for (const project of PROJECT_NAVIGATION) {
    test(`${project.route} shares bilingual tools, exhibit links and a single return`, async ({ page }, testInfo) => {
      for (const locale of ["en", "zh"] as const) {
        ...
        } else if (project.slug === "groupconv-atlas") {
          await expect(stamp).toHaveText(locale === "zh" ? "实测证据" : "MEASURED EVIDENCE");
  ...
  test("SEARCH ⌘K opens the command palette from the Privacy auto-rail route", async ({ page }) => {
  ...
  test("EN·中 toggle switches language and preserves the page's path and hash (spec §2.5)", async ({ page }) => {
    await page.goto("/projects/triage-router#exhibit-02", { waitUntil: "networkidle" });
    await expect(page).toHaveURL(/\/projects\/triage-router#exhibit-02$/);
    await page.locator(SEL.projectRailTools).getByRole("button", { name: "中" }).click();
    await expect(page).toHaveURL(/\/projects\/triage-router\?lang=zh#exhibit-02$/);
    ...
    await expect(page.locator(".circuit-pos")).toHaveText("度量与决策 · 01 / 05");
    ...
    await expect(page.locator(".circuit-pos")).toHaveText("MEASURE & DECIDE · 01 / 05");
  });
  ...
  test("section positions and active anchors stay aligned through a locale switch under reduced motion", async ({ page }, testInfo) => {
    ...
    await page.goto("/projects/triage-router?lang=zh#exhibit-02", { waitUntil: "networkidle" });
  ...
  // /ai/frontier-forge (that route's URL at the time; /projects/frontier-forge
  // today) with a single content click. Fixed by giving the
