...
const stackRoutes = [
  "/projects/frontier-forge", "/projects/frontier-forge", "/projects/exactly-once-drills",
  "/projects/exactly-once-drills", "/projects/crossover-study", "/projects/release-guardian",
];
...
for (const locale of ["en", "zh"] as const) {
  ...
  test(`stack project links (${locale}) are localized, keyboard operable and leave layer labels as text`, async ({ page }) => {
    ...
    await expect(page).toHaveURL(new RegExp(`/projects/frontier-forge${locale === "zh" ? "\\?lang=zh" : ""}$`));
  ...
  test(`stack project links (${locale}) exist without JavaScript`, async ({ browser, baseURL }) => {
    ...
    await expect(page).toHaveURL(/\/projects\/crossover-study$/);
    await context.close();
  });

  for (const [route, bank] of Object.entries(questionBank)) {
    // The tandem compatibility page is a noindex shell with no launcher
    // preset surface of its own; its bank and alias routing remain
    // exhaustively covered by project-identities.test.mjs.
    if (route === "/projects/analytics-tandem") continue;
  ...
  test(`Ask overview answers (${locale}) link all names and project-level destinations`, async ({ page }) => {
    let calls = 0;
    await page.route("**/api/assistant", async (request) => { calls += 1; await request.abort(); });
    await page.goto(`/projects/ask-portfolio${suffix}`, { waitUntil: "networkidle" });
    for (const [index, question] of questionBank["/"].questions.entries()) {
      ...
      for (const citation of record.citations) {
        if(citation.url) await expect(answer.locator(`.ask-go a[href="${citation.url}"]`)).toHaveCount(0);
      }
      const destinations: Record<string, string[]> = {
        "home-background": ["/", "/projects/frontier-forge", "/projects/release-guardian"],
        "home-site-overview": ["/projects/frontier-forge", "/projects/release-guardian", "/projects/triage-router", "/projects/exactly-once-drills"],
        "home-tech-stack": ["/", "/projects/frontier-forge", "/projects/release-guardian", "/projects/triage-router"],
      };
      expect(await answer.locator(".ask-go a").evaluateAll(nodes=>nodes.map(node=>node.getAttribute("href"))))
        .toEqual(destinations[question.id].map(path=>`${path}${suffix}`));
    }
    expect(calls).toBe(0);
  });
  ...
  test(`known legacy project failure (${locale}) offers the requested routes`, async ({ page }) => {
    ...
    expect(await links.evaluateAll((nodes) => nodes.map((node) => node.getAttribute("href"))))
      .toEqual([`/projects/exactly-once-drills${suffix}`, `/projects/credit-policy-desk${suffix}`]);
  });
  ...
  test(`both answer surfaces (${locale}) render all typed and legacy names as canonical links`, async ({ page }) => {
    ...
    const expected = [...mentions.map(({ id }) => projectIdentityHref(id, locale)), `/projects/exactly-once-drills${suffix}`];
    await page.goto(`/projects/ask-portfolio${suffix}`, { waitUntil: "networkidle" });
