...
test("homepage canonicalization keeps client navigation and Back/Forward in the same document", async ({ page }) => {
  ...
  await expect(page).toHaveURL("/projects/frontier-forge");
  await page.goBack();
  await expect(page).toHaveURL("/#contact");
  await expectLocale(page, "en");
  await page.goForward();
...
test("Artifact Viewer retains explicit English, strict query parsing, and localized return links", async ({ page }) => {
  ...
  // The URL a stale link still carries: `from` names the retired /ai route.
  // The viewer resolves it through src/lib/artifacts.ts's alias map, so the
  // link keeps working AND the address bar canonicalizes to the flat route --
  // `href` below is what the viewer must rewrite that stale URL into.
  const staleHref = (lang: string) => `/artifact?src=${encodeURIComponent(source)}&from=%2Fai%2Frag-quality-lab&${lang}#records`;
  const href = (lang: string) => `/artifact?src=${encodeURIComponent(source)}&from=%2Fprojects%2Frag-quality-lab&${lang}#records`;
  await page.goto(staleHref("lang=en"));
  await expectLocale(page, "en");
  await expect(page).toHaveURL(href("lang=en"));
  await expect(page.locator(".artifact-page > .back-link")).toHaveAttribute("href", "/projects/rag-quality-lab");
  await page.reload();
  await expectLocale(page, "en");
  await expect(page).toHaveURL(href("lang=en"));
  ...
  for (const query of ["lang=zh-CN", "lang=zh&lang=en"]) {
    ...
    await page.goto(staleHref(query));
    await expectLocale(page, "en");
    await expect(page).toHaveURL(href("lang=en"));
  }
  await page.getByRole("button", { name: "中", exact: true }).click();
  ...
  await expect(page.locator(".artifact-page > .back-link")).toHaveAttribute("href", "/projects/rag-quality-lab?lang=zh");
});
