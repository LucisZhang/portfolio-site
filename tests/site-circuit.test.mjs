...
const CHAIN = [
  { number: "01", slug: "frontier-forge", href: "/ai/frontier-forge", group: "build-run", position: { en: "BUILD & RUN · 01 / 02", zh: "构建与运行 · 01 / 02" } },
  { number: "02", slug: "exactly-once-drills", href: "/engineering/exactly-once-drills", group: "build-run", position: { en: "BUILD & RUN · 02 / 02", zh: "构建与运行 · 02 / 02" } },
  { number: "03", slug: "release-guardian", href: "/ai/release-guardian", group: "guard-verify", position: { en: "GUARD & VERIFY · 01 / 04", zh: "把关与验证 · 01 / 04" } },
  { number: "04", slug: "privacy-preflight", href: "/ai/privacy-preflight", group: "guard-verify", position: { en: "GUARD & VERIFY · 02 / 04", zh: "把关与验证 · 02 / 04" } },
  { number: "05", slug: "rag-quality-lab", href: "/ai/rag-quality-lab", group: "guard-verify", position: { en: "GUARD & VERIFY · 03 / 04", zh: "把关与验证 · 03 / 04" } },
  { number: "06", slug: "ask-portfolio", href: "/ai/ask-portfolio", group: "guard-verify", position: { en: "GUARD & VERIFY · 04 / 04", zh: "把关与验证 · 04 / 04" } },
  { number: "07", slug: "triage-router", href: "/ai/triage-router", group: "measure-decide", position: { en: "MEASURE & DECIDE · 01 / 04", zh: "度量与决策 · 01 / 04" } },
  { number: "08", slug: "crossover-study", href: "/engineering/crossover-study", group: "measure-decide", position: { en: "MEASURE & DECIDE · 02 / 04", zh: "度量与决策 · 02 / 04" } },
  { number: "09", slug: "margin-control-tower", href: "/analytics/margin-control-tower", group: "measure-decide", position: { en: "MEASURE & DECIDE · 03 / 04", zh: "度量与决策 · 03 / 04" } },
  { number: "10", slug: "credit-policy-desk", href: "/analytics/credit-policy-desk", group: "measure-decide", position: { en: "MEASURE & DECIDE · 04 / 04", zh: "度量与决策 · 04 / 04" } },
];
...
test("every stop carries its family and position readout in both locales", () => {
  ...
  assert.deepEqual(circuitStops.map((stop) => stop.indexInGroup), [1, 2, 1, 2, 3, 4, 1, 2, 3, 4]);
...
test("the taxonomy holds three families with the expected labels, glosses, and members", () => {
  ...
  assert.deepEqual(circuitGroups.map((group) => group.members.length), [2, 4, 4]);
  assert.deepEqual(circuitGroups.map((group) => group.members), [
    ...
    ["triage-router", "crossover-study", "margin-control-tower", "credit-policy-desk"],
  ]);
...
test("prev/next is circular over all ten stops and null off the circuit", () => {
  ...
  assert.deepEqual(crossings, ["exactly-once-drills", "ask-portfolio", "credit-policy-desk"]);
  // The wrap, spelled out: 10 -> 01 and 01 -> 10.
  assert.equal(circuitEntryForPath("/analytics/credit-policy-desk").next.slug, "frontier-forge");
  assert.equal(circuitEntryForPath("/ai/frontier-forge").prev.slug, "credit-policy-desk");
  // Trailing slashes normalize; off-circuit routes render no circuit at all.
  assert.equal(circuitEntryForPath("/ai/frontier-forge/").stop.slug, "frontier-forge");
  assert.equal(circuitEntryForPath("/"), null);
  assert.equal(circuitEntryForPath("/analytics/analytics-tandem"), null);
