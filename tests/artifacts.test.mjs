...
test("from only retains the owning project or known home targets and canonicalizes old routes", () => {
  const froms = ["//evil.test", "/\\evil.test", "https://evil.test", "javascript:alert(1)", "/api/assistant", "/artifact", "/ai/frontier-forge", "/ai/../ai/rag-quality-lab", "/%61i/rag-quality-lab", "/ai/rag-quality-lab\n"];
  for (const from of froms) assert.equal(resolve({ src: rag, from }).from, "/ai/rag-quality-lab", from);
  assert.equal(resolve({ src: rag, from: "/ai/rag-quality-lab/?lang=zh&next=//evil.test#unknown" }).from, "/ai/rag-quality-lab");
  assert.equal(resolve([["src", rag], ["from", "/"], ["from", "/ai/frontier-forge"]]).from, "/ai/rag-quality-lab");
  ...
  for (const [directory, from, target] of [
    ["exactly-once-drills", "/engineering/p1-reliability-lab", "/engineering/exactly-once-drills"],
    ["credit-policy-desk", "/analytics/credit-policy-lab", "/analytics/credit-policy-desk"],
    ["privacy-preflight", "/ai/privacy-preflight-mac", "/ai/privacy-preflight"],
  ]) {
...
test("lang is exact, unambiguous, independent of from/src, and shareable in both languages", () => {
  for (const lang of ["ZH", "zh-CN", "zh ", "en-US", "javascript:zh", "", "%7ah", "zh&lang=en"]) {
    assert.equal(resolve({ src: rag, from: "/ai/rag-quality-lab?lang=zh", lang }, "zh").locale, "en");
  }
  ...
  assert.equal(resolve({ src: rag, from: "/ai/rag-quality-lab?lang=zh", lang: "en" }).locale, "en");
});

test("all callers generate valid context; nested Markdown links remain allowlisted", () => {
  for (const from of ["/", "/ai/rag-quality-lab", "/artifact", "//evil.test"]) {
