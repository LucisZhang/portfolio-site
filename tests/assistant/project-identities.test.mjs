...
const matrix = {
  "groupconv-atlas": "/projects/groupconv-atlas",
  "frontier-forge": "/projects/frontier-forge",
  "release-guardian": "/projects/release-guardian",
  "exactly-once-drills": "/projects/exactly-once-drills",
  "rag-quality-lab": "/projects/rag-quality-lab",
  "triage-router": "/projects/triage-router",
  "privacy-preflight": "/projects/privacy-preflight",
  "margin-control-tower": "/projects/margin-control-tower",
  "crossover-study": "/projects/crossover-study",
  "ask-portfolio": "/projects/ask-portfolio",
  "credit-policy-desk": "/projects/credit-policy-desk",
  "analytics-tandem": "/#archive",
  "Voice-in-Security": "https://github.com/LucisZhang/Voice-in-Security",
};
// Route resolves to the flat page; href sends readers to the archive shelf instead.
const ARCHIVE_ONLY = new Set(["analytics-tandem"]);
...
test("every published project has one reviewed semantic identity and catalog title", () => {
  ...
  for (const project of routableProjects) {
    ...
    assert.equal(identity.route, `/projects/${project.slug}`);
    assert.deepEqual(identity.label, project.title);
  }
});

test("project routes are flat and old track routes still resolve", () => {
  for (const [id, identity] of Object.entries(PROJECT_IDENTITIES)) {
    if (identity.kind !== "portfolio") continue;
    assert.equal(identity.route, `/projects/${id}`, `${id}: flat route`);
    if (ARCHIVE_ONLY.has(id)) {
      assert.equal(identity.href, "/#archive", `${id}: archive href stays on the shelf`);
    } else {
      assert.equal(identity.href, identity.route, `${id}: href matches route`);
    }
  }
  assert.equal(resolveProjectIdentity("/ai/frontier-forge").href, "/projects/frontier-forge");
  assert.equal(resolveProjectIdentity("/engineering/exactly-once-drills").href, "/projects/exactly-once-drills");
  assert.equal(resolveProjectIdentity("/analytics/credit-policy-desk").href, "/projects/credit-policy-desk");
  assert.equal(resolveProjectIdentity("/analytics/credit-policy-lab").href, "/projects/credit-policy-desk");
  assert.equal(resolveProjectIdentity("/projects/frontier-forge").href, "/projects/frontier-forge");
  assert.equal(resolveProjectIdentity("/analytics/analytics-tandem").href, "/#archive");
  assert.equal(resolveProjectIdentity("/projects/analytics-tandem").href, "/#archive");
});
    ...
    for (const question of entry.questions) {
      ...
      for (const citation of question.answer.citations) {
        if (citation.site && !citation.file) {
          const base = citation.site === "/" ? "/" : matrix[resolveProjectIdentity(citation.site).id];
          // Task B5: an authored {site, anchor} citation deep-links to one
          // exhibit section of its destination, so the index entry carries
          // the fragment (src/lib/assistant-citation-index.ts siteEntry()).
          // An href that already ends in a fragment — the ARCHIVE_ONLY case
          // — keeps its own rather than growing a second one.
          const href = citation.anchor && !base.includes("#") ? `${base}#${citation.anchor}` : base;
...
test("generator rejects misrouted names, duplicate presets, unknown citations and raw answer URLs", () => {
  const badBanks = [
    (copy) => { copy["/projects/triage-router"].questions[0].q_en = "What does Release Guardian do?"; },
    (copy) => { copy["/projects/triage-router"].questions[0].q_zh = "Credit Policy Lab 为什么说一个分数还不是策略？"; },
