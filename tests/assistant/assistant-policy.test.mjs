...
test("dedicated LLM guard sees only the question and page context, blocks algebra, and runs before retrieval", async () => {
  const guardPayload = buildAssistantGuardPayload(DEFAULT_ASSISTANT_GUARD_MODEL, {
    locale: "zh",
    pageContext: "/projects/rag-quality-lab",
...
test("payload uses locale-specific model, ZDR routing, structured citations, and bounded history", () => {
  ...
  assert.deepEqual(payload.response_format.json_schema.schema.properties.blocks.items.properties.segments.items.anyOf[1].properties.projectId.enum, [
    "groupconv-atlas", "frontier-forge", "release-guardian", "exactly-once-drills", "rag-quality-lab", "triage-router", "privacy-preflight",
    "margin-control-tower", "crossover-study", "ask-portfolio", "credit-policy-desk", "analytics-tandem", "Voice-in-Security",
  ]);
