...
Each substantive project has one bilingual report index and one region for
Architecture, Results and Limitations, in that document order. The stable
fragment targets are `#report-architecture`, `#report-results` and
`#report-limitations`. Plain links preserve the active URL and work without
JavaScript. Their target headings accept keyboard focus and clear the mobile
navigation bar. Existing exhibit IDs remain available.
...
order, unique accessible headings, keyboard fragment navigation, reading-column
geometry, overflow, catalog boundary/outcome/negative text and no-JavaScript
navigation. It separately checks Privacy's single boundary list, Crossover's
embedded SQL instrument and Ask's recorded refusals.
