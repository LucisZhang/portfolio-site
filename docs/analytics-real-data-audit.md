...
The fixed-seed fixtures remain the default and fallback. Real mode is an explicit user action that
loads only committed, derived ZSTD Parquet through DuckDB-WASM. Both Python pipelines fail closed
on source hashes and browser contracts, embed provenance metadata, and emit bilingual on-page
methods evidence. No raw upstream dataset is committed.
