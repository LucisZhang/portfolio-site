...
| --- | --- | --- |
| `86,972` | `$SOURCE_ROOT/nlp-eval-lab/demo/data/calibration.json` | `$.exhibits[0].n` (`label`: "Tier A LogReg — raw probabilities (CAL)", `slice`: `"cal"`) — the calibration-slice size the threshold sweep behind `policies.compact.json` (task 3.0's `a_to_human__full_cal__…` source, registered in `r2-source-map.md` row 11) was fit on |
...
| `2026-08-12` | `$SOURCE_ROOT/nlp-eval-lab/demo/data/meta.json` `$.headline_router.note` ("Owner decision 2026-08-12") and `drift.json`'s own `$.generated_at` (`2026-08-12T12:21:24.484273Z`) | same evidence-generation batch as the grid |
...
`$SOURCE_ROOT/nlp-eval-lab/demo/live/tier_b2/` (the four
