...
| Margin Control Tower | Browser implementation is public inside the Portfolio review branch; current pipeline upgrade remains local | Same Portfolio branch / PR for browser code; active local integration branch for pipeline candidate | Portfolio rights apply; dataset terms recorded separately | Fixed-seed fixture plus derived-only public-dataset candidate; raw source excluded | Current local pipeline/history require fresh scans | Public browser source link; real-data pipeline publication remains pending | Verify provenance/license/hash log, authorize exact candidate, push, then verify anonymously |
| Credit Policy Lab | Browser implementation is public inside the Portfolio review branch; current pipeline upgrade remains local | Same Portfolio branch / PR for browser code; active local integration branch for pipeline candidate | Portfolio rights apply; dataset terms recorded separately | Fixed-seed fixture plus derived-only public-dataset candidate; raw source excluded | Current local pipeline/history require fresh scans | Public browser source link; real-data pipeline publication remains pending | Verify provenance/license/hash log, authorize exact candidate, push, then verify anonymously |
