...
| Email, phone, URL, IPv4, local path, known secret, long ID | Yes | Complete for deterministic subset | TypeScript rules ported from the deterministic Mac detector |
...
| Local image OCR and detected boxes | Vision / OCR worker | Complete | Tesseract worker, quantized English/Chinese data, and WASM are generated as same-origin assets; rule-matched word boxes enter review |
...
| PDF page navigation / per-page review | Yes | Complete | Previous/next controls, page-specific OCR/manual regions, and an explicit reviewed state for every page |
| Destructive image-only PDF export | Yes | Complete vertical slice | Every page is rendered, accepted regions are burned black, and pdf-lib builds a new image-only PDF |
